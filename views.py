...
def home(request, name):
    fundraiser = Event.objects.get(name='the wulf. 2014 fundraiser')
...
def events(request, name):
    return events_by_year(request, name, '2015')
...
def donate(request, name):
    fundraiser_event = Event.objects.get(name = 'the wulf. 2014 fundraiser')
